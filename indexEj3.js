...
const winnerByYear = (arr, year) => {
  const groupYear = arr.filter((element)=>element.year===year);
  const sortYear = groupYear.sort((a,b)=>b.ranking-a.ranking);
  const topWinner= sortYear.slice(0,3);
  const orderWinner= topWinner.sort((a,b)=>b.year-a.year)
  const winners = orderWinner.map((element)=>element.name)
  return winners;
};
